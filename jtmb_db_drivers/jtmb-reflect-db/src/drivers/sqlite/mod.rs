...
async fn meta_fn_sqlite_master<Executor, Database>(
    executor: Executor,
    object_type: String,
    name: String,
    table_name: String,
    rootpage: i64,
) -> Result<Vec<SqliteMasterInfo>, anyhow::Error>
where
    // database bounds
    Database: sqlx::Database,
    //executor bounds
...
pub async fn sqlite_meta_fn_sqlite_master<Executor>(
    executor: Executor,
    object_type: String,
    name: String,
    table_name: String,
...
{
    meta_fn_sqlite_master::<Executor, sqlx::Sqlite>(
        executor,
        object_type,
        name,
        table_name,
...
pub async fn txn_meta_fn_sqlite_master<Database>(
...
where
    Database: sqlx::Database,
    //executor bounds
    for<'c> &'c mut <Database as sqlx::Database>::Connection:
        sqlx::Executor<'c, Database = Database>,
...
{
    let c = **transaction;
    meta_fn_sqlite_master::<&mut <Database as sqlx::Database>::Connection, Database>(
        &mut **transaction,
        object_type,
        name,
        table_name,
...
impl<'a> crate::SchemaInspector<sqlx::Sqlite> for sqlx::Transaction<'a, sqlx::Sqlite> {
    ...
    async fn get_columns(
        ...
        let t = sqlite_meta_fn_get_columns_for_table_with_datatype(
            &mut **self,
            for_table_name.to_string(),
            "".to_string(),
            "".to_string(),
            true,
            i64::MAX,
...
async fn c() -> Result<sqlx::SqliteConnection, anyhow::Error> {
    sqlx::SqliteConnection::connect("::memory::")
        .await
        .map_err(|e| e.into())
}
