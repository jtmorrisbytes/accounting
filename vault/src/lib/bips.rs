...
pub fn bips_39() -> Result<Vec<String>, Box<dyn std::error::Error>> {
    ...
    for i in 0..entropy.len() {
        unsafe {
            std::ptr::write_volatile(&mut entropy[i], 0);
        }
    }
    ...
    {
        unsafe {
            windows::Win32::System::Memory::VirtualUnlock(entropy.as_ptr().cast(), entropy.len())?;
        }
    }
    ...
    for i in 0..all_bits.len() {
        unsafe {
            std::ptr::write_volatile(&mut all_bits[i], 0);
        }
    }
    std::sync::atomic::compiler_fence(std::sync::atomic::Ordering::SeqCst);
    Ok(mnemonic)
